...
const Hero = ({ searchQuery }) => {
  ...
  // Debounce: only trigger fetch after typing stops for 500ms

const fetchBooks = async (query, isFallback = false) => {
  try {
    setLoading(true);

    const response = await fetch(
      `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(query)}&maxResults=35&key=AIzaSyA6GXZvJsHFoDFedybxgNLbIc3Hh9JC3Mk`
    );
    ...
    return (
      <div className="animate-pulse grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 p-6">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="h-64 md:h-96 bg-gray-900/50 rounded-xl md:rounded-2xl backdrop-blur-lg"
          />
        ))}
      </div>
    );
...
return (
  <div
    id="hero"
    key={book.id}
    className="group relative bg-gray-800 rounded-xl md:rounded-2xl backdrop-blur-xl border border-white/10 hover:border-cyan-600/30 transition-all duration-300 hover:-translate-y-1 md:hover:translate-y-2 shadow-lg hover:shadow-xl md:shadow-2xl hover:shadow-cyan-400/10"
  >
    <a
      href={book.infoLink}
      target="_blank"
      ...
      className="block w-full h-full rounded-xl md:rounded-2xl"
    >
        ...
        <div className="mt-4 md:mt-6">
          ...
          <div className="flex items-center mt-2 md:mt-3">
            <div className="flex text-amber-400">
              {[...Array(5)].map((_, i) => (
                <StarIcon
                  key={i}
                  className={`h-4 w-6 md:h-5 md:w-5 ${
          ...
          <div className="mt-2 md:mt-4 flex flex-wrap gap-1 md:gap-2">
            {book.categories.split(",").slice(0, 3).map((category, index) => (
              <span
                key={index}
                className="px-2 py-1 md:px-3 md:py-3 rounded-full bg-gray-700/50 text-xs text-cyan-300 backdrop-blur-sm"
              >
                {category.trim()}
    ...
    <div className="absolute inset-0 rounded-xl md:rounded-2xl bg-gradient-to-br from-cyan-400/20 to-blue-500/20 opacity-0 group-hover:opacity-30 transition-opacity duration-300 pointer-events-none" />
        ...
        <div className='mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-3'>
         ...
         disabled = {currentPage === 1}
          className='px-1 py-1 md:px-4 md:py-2 rounded-md md:rounded-lg bg-gray-500/20 border
          border-gray-700/30 text-cyan-300 text-xs md:text-sm hover:hg-gray-900/60 transition-colors
          disabled :opacity-50'>
            Previous
         </button>
         ...
         {generatePageNumber().map((page, index) => (
          ...
          className={`px-1 py-1 md:px-4 md:py-2 rounded-md md:rounded-lg text-xs md:text-sm ${page === currentPage
